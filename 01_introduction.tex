...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/intro/bee_spider.png}
    \caption*{\textbf{Guêpe prise dans une toile d'araignée.}
    Les deux animaux sont des prédateurs qui doivent se nourrir pour survivre.
    Ici, la guêpe pourrait ou non devenir la nourriture de l'araignée, cette
    incertitude pouvant être représentée par une probabilité. Ces deux animaux
    font partie d'une communauté plus large d'espèces qui interagissent entre
    elles à un endroit et moment particuliers, formant un réseau trophique d'interactions
    prédateurs-proies et plantes-herbivores. Cette thèse décrit l'incertitude au
    sein des réseaux trophiques, notamment la probabilité qu'une interaction
    entre deux espèces se produise localement. En outre, cette thèse montre
    comment prédire la structure émergente des réseaux trophiques lorsque nous
    avons peu d'information sur ces interactions. Crédit photo~: Yves Banville.}
\end{figure}
...
proie, un parasite et son hôte ou une plante et son pollinisateur. Ils nous
permettent de mieux comprendre la fonction écologique des espèces (p. ex. la
régulation de la taille des populations de chenilles par la mésange charbonnière
\textit{Parus major}, \cite{Harvey2017Bridging}) et leur rôle dans la communauté
(p. ex. le rôle des espèces clés de voûte, telle que la loutre de mer
\textit{Osmia cornuta}, dans le fonctionnement et la stabilité des écosystèmes,
\cite{Estes1998Killer}), notamment à l'aide de mesures sur l'influence des
espèces dans le réseau (p. ex. le nombre d'espèces avec qui elles
interagissent). Bien ancrée dans la science des réseaux et la théorie des
graphes, l'écologie des réseaux fournit un cadre d'analyse pour étudier les
propriétés émergentes des écosystèmes, comme leur fonctionnement (p. ex.
productivité primaire et cycle des nutriments) et résilience face aux
changements environnementaux (\cite{Proulx2005Network},
\cite{McCann2007Protecting}, \cite{McCann2011Food},
\cite{Rooney2012Integrating}, \cite{Valiente-Banuet2019Species}). Pour ce faire,
elle a recours à des mesures de la structure émergente des réseaux
d'interactions (\cite{Delmas2019Analysing}). Par exemple, la structure modulaire
d'un réseau (structure où les interactions sont nombreuses au sein de groupes
d'espèces, mais peu nombreuses entre les groupes) favorise la stabilité d'un
écosystème en limitant la propagation des perturbations, telles que les
extinctions d'espèces (\cite{Stouffer2011Compartmentalization}) et la présence
de contaminants (\cite{Garay-Narvaez2014Food}), à l'ensemble du réseau
(\cite{May1972Will}). Les mesures des réseaux d'interactions entre espèces sont
donc essentielles pour mieux comprendre les caractéristiques des systèmes
...
à partir d'un nombre limité de variables écologiques, tout en nous aidant à
mieux comprendre les déterminants de ces réseaux complexes. Avant d'exposer
cette théorie, il convient de passer en revue quelques caractéristiques
importantes des systèmes écologiques complexes, notamment en ce qui a trait aux
déterminants des interactions entre espèces, aux mesures des
interactions entre espèces et à l'analyse de la structure des réseaux.
J'explique notamment, dans les sous-sections suivantes, que les interactions
entre espèces sont le résultat de nombreux mécanismes écologiques souvent
difficiles à départager, que les interactions entre espèces sont intrinsèquement
probabilistes et que la structure émergente des réseaux d'interactions est
écologiquement et statistiquement contrainte. Ces observations sous-tendent la
théorie proposée en décrivant des aspects fondamentaux de la complexité
écologique.

\subsection{Quels sont les déterminants et mécanismes des interactions entre espèces?} 
...
important d'identifier les déterminants (c.-à.-d. les variables ou facteurs
influençant un phénomène) et mécanismes (c.-à-d. les processus ou règles qui
expliquent un phénomène) écologiques des interactions entre espèces pour mieux
comprendre les conditions favorisant leur réalisation. La théorie de l'entropie
maximale des réseaux trophiques, mise de l'avant dans cette thèse, repose sur un
ensemble restreint de variables écologiques importantes déterminant la structure
des réseaux trophiques. Elle peut guider nos efforts d'identification des
mécanismes écologiques importants en orientant nos recherches vers ses erreurs
de prédiction et variables utilisées. Cependant, notre compréhension de ces
déterminants et mécanismes peut dépendre du type d'interaction, du niveau
d'organisation taxonomique et de l'échelle spatiale des réseaux étudiés. 
...
d'aborder en parallèle leurs déterminants écologiques (tel qu'effectué dans
l'article~\ref{art:article1}, qui présente les variables déterminant différents
types d'interactions entre espèces).

Les interactions prédateurs-proies et plantes-herbivores ont toutefois une
...
dans l'article~\ref{art:article1}, comprendre comment les facteurs écologiques
...
interactions observées (p. ex. \cite{Maiorano2020Tetraeu}) et/ou prédites (p.
ex. \cite{Strydom2022Food}) entre un groupe d'espèces données. Contrairement aux
...
presqu'exclusivement sur des règles de correspondance de traits entre les
espèces. 

Les réseaux locaux (\cite{Poisot2012Dissimilarity}), quant à eux, sont
constitués d'interactions observées et/ou prédites d'être réalisées à un endroit
et moment particuliers. Ce sont eux qui décrivent les échanges directs
d'information, de matière ou d'énergie réalisés par les individus à l'échelle
locale. Étant donné qu'une interaction locale ne peut être réalisée que si elle
est biologiquement faisable, les réseaux locaux sont des sous-réseaux du
\textit{metaweb} (\cite{Saravia2022Ecological}). La
Figure~\ref{fig:reseaux_concepts} compare visuellement un réseau régional fictif
à deux réseaux locaux (réalisés et observés). Deux espèces interagissent
...
\cite{Canard2014Empirical}) et que leurs traits supportent une interaction
lorsqu'elles se rencontrent (\cite{Bolnick2011Why}, \cite{Gravel2013Inferring},
\cite{Stouffer2011Role}). Les interactions locales sont donc en partie basées
sur des mécanismes liés à la rencontre (p. ex. mouvement, reproduction,
mortalité) et à la correspondance de traits des individus. Par exemple, les
habitudes alimentaires du cerf de Virginie varient localement selon la
disponibilité saisonnière en nourriture (\cite{Short1975Nutrition}) et sa masse
corporelle (\cite{Luna2013Influence}). Les conditions environnementales, comme
la température (\cite{Angilletta2004Temperature}), les précipitations
...
des interactions locales (\cite{Poisot2015Species}, article~\ref{art:article1}).
Les mécanismes sous-tendant les interactions locales sont nombreux, et il peut
être difficile d'identifier lesquels contribuent le plus à la réalisation des
interactions. Il peut donc être hasardeux de construire une théorie des réseaux
trophiques basée sur des mécanismes particuliers.  

\afterpage{\clearpage

\begin{figure}[!h]
  \centering
  \includegraphics[width=\textwidth]{figures/intro/reseaux_concepts.png}
  \caption{\textbf{Comparaison entre les interactions régionales et locales de réseaux trophiques fictifs.}
  Un réseau régional (gauche) est un catalogue d'interactions potentielles
  pouvant être réalisées localement ou non. Les réseaux locaux (droite)
  contiennent l'ensemble des interactions se produisant (réseaux locaux
  réalisés) ou ayant été échantillonnées (réseaux locaux observés) à un endroit
  et moment particuliers (ici, à deux endroits en Afrique subsaharienne). Les
  réseaux locaux réalisés peuvent contenir des interactions observées et/ou
  prédites. Puisqu'il peut y avoir des espèces ou des interactions manquantes
  (espèces présentes ou interactions réalisées qui n'ont pas été observées) dans
  les réseaux observés, ceux-ci sont un sous-ensemble des réseaux réalisés. De
  même, les espèces présentes et interactions réalisées localement (prédites ou
  observées) sont un sous-ensemble des espèces et interactions régionales. La
  position des nœuds est la même dans les différents réseaux pour faciliter leur
  comparaison. Deux espèces (\textit{Panthera leo} et \textit{Suricata
  suricatta}) sont mises en évidence pour mieux comparer leurs interactions dans
  différents réseaux. Bien que les réseaux trophiques soient des réseaux dirigés
  (c.-à-d. des réseaux dont les flux de matière et d'énergie se font dans un
  sens particulier), la direction des interactions n'est pas représentée
  graphiquement pour simplifier la représentation des réseaux.}
\label{fig:reseaux_concepts}
\end{figure}
}
...
\subsubsection{Les interactions entre espèces sont intrinsèquement probabilistes} 
...
Les interactions probabilistes, qui sont décrites en détail dans
l'article~\ref{art:article1}, mesurent généralement notre niveau de croyance
quant à la présence d'une interaction entre espèces
(\cite{Poisot2016Structure}). À l'échelle locale, elles représentent la
probabilité qu'une interaction soit réalisée à un endroit et moment
particuliers. Différents types de modèles peuvent être utilisés pour estimer la
probabilité que deux espèces interagissent localement, tels que des modèles de
régression logistique ou d'apprentissage automatique
(\cite{Strydom2021Roadmapa}). Ces modèles peuvent prendre en compte différentes
variables biologiques ou environnementales mesurées à l'échelle locale. Par
exemple, si nous n'avons pas observé d'interaction entre un cerf de Virginie et
\textit{I. scapularis} dans une forêt donnée, nous pouvons évaluer la
probabilité que ces deux espèces interagissent localement à partir de leurs
abondances relatives (\cite{Canard2014Empirical}). Nous pouvons également
utiliser notre expertise sur la biologie des deux espèces pour choisir une
probabilité \textit{a priori} informative (c.-à-d. une probabilité reposant sur
nos connaissances antérieures à nos observations locales) que l'interaction soit
réalisée localement. Cette probabilité \textit{a priori} peut ensuite être mise
à jour à l'aide de méthodes d'inférence bayésienne en considérant les conditions
locales. Au-delà des modèles statistiques, la probabilité d'une interaction
locale peut aussi être estimée en comptant le nombre de fois où elle a été
observée au cours d'un certain nombre d'essais. Par exemple, si on échantillonne
aléatoirement un certain nombre de réseaux locaux dans une région donnée et que
ces réseaux sont représentatifs de la région, la proportion des réseaux où deux
espèces données interagissent est un bon estimateur de la probabilité qu'elles
interagissent dans un autre réseau (non échantillonné) de la région. À l'échelle
régionale, les interactions probabilistes représentent plutôt la probabilité
qu'une interaction soit biologiquement ou écologiquement faisable
(\cite{Strydom2023Grapha}). Cette probabilité d'interaction régionale est
particulièrement utile lorsque deux espèces n'ont jamais été observées en train
d'interagir, mais qu'elles pourraient possiblement le faire dans les bonnes
circonstances sur la base de leurs traits. L'article~\ref{art:article1} présente
de manière plus approfondie les méthodes et variables permettant d'estimer les
interactions probabilistes locales et régionales.

Qu'elles soient locales ou régionales, les interactions probabilistes sont
généralement évaluées de manière indépendante. Cela peut introduire des biais
lorsque nous propageons l'incertitude des interactions locales vers la structure
du réseau (\cite{Poisot2016Structure}), à cause de l'impact que les interactions
peuvent exercer les unes sur les autres au sein d'un réseau local
...
temps ou sa complexité comportementale (\cite{Strydom2021Svd}). 
...
complexité interne d'un réseau. L'entropie est une mesure du niveau moyen
d'incertitude (ou d'information) associée aux différentes issues d'une variable
aléatoire. L'entropie de Shannon $H$ (\cite{Shannon1948Mathematical}) est la
mesure d'entropie la plus courante en théorie de l'information, et est calculée
exclusivement à partir de la distribution de probabilité $p(x)$ de la variable
pour différentes issues $x$ : 

\begin{eqnarray}
    \label{eq:shannon_intro}
            H = -\sum_{x} p(x) \log p(x).
\end{eqnarray}

L'entropie est maximale lorsque toutes les issues ont la même probabilité,
c.-à-d. lorsque la distribution est uniforme et que notre incertitude sur le
résultat de l'événement est la plus élevée. L'entropie SVD est calculée en
appliquant l'entropie de Shannon aux valeurs singulières relatives de la matrice
d'adjacence (obtenues par SVD). Elle quantifie l'information contenue dans les
rangs de la matrice d'adjacence, c.-à-d. la quantité d'information requise pour
encoder le réseau. Une entropie SVD élevée indique que les rangs de la matrice
contiennent une quantité similaire d'information, et donc que la structure du
réseau est peu compressible (\cite{Gu2016How}). Cette mesure constitue une façon
plus robuste de quantifier la complexité d'un réseau que les autres mesures
mentionnées plus haut (\cite{Strydom2021Svd}). En effet, l'entropie SVD est la
seule de ces mesures capturant la complexité physique d'un système (c.-à-d. la
quantité d'information nécessaire pour encoder le réseau), les autres capturant
plutôt sa complexité comportementale (c.-à-d. sa faible prédictibilité dans le
temps). \textcite{Adami2002What} suggère de mesurer la complexité d'un système
par sa complexité physique plutôt que comportementale, cette dernière pouvant
facilement émerger d'un système simple (p. ex. \cite{May1976Simple}).
Puisqu'elle évalue la quantité d'information contenue dans un réseau (ou sa
compressibilité), l'entropie SVD a été choisie comme mesure de complexité
écologique dans les versions de la théorie de l'entropie maximale des réseaux
trophiques développées dans l'article~\ref{art:article2} de cette thèse. 


\subsubsection{Plusieurs mesures de la structure des réseaux sont corrélées entre elles} 

La structure des réseaux d'interactions entre espèces est caractérisée par des
...
\subsubsection{Une théorie adaptée aux réseaux locaux} 
...
prédictions nous permet de mieux comprendre les facteurs importants qui
déterminent la structure des réseaux trophiques. Ainsi, en tant que théorie
scientifique, elle permet à la fois de générer des prédictions vérifiables sur
la structure des réseaux trophiques et de mieux comprendre leurs déterminants
écologiques. 
...
\subsubsection{Guider notre recherche des mécanismes écologiques importants} 

Bien que cette théorie ne soit pas basée sur des mécanismes écologiques
explicites, elle peut guider nos efforts d'identification des mécanismes les
plus importants régissant les réseaux trophiques. Premièrement, la valeur des
variables d'état capture les mécanismes écologiques sous-jacents aux réseaux
(\cite{White2012Characterizing}, \cite{McGill2010Mechanisms}). Par exemple, si
une version de la théorie utilisant le nombre d'espèces et le nombre
d'interactions comme variables d'état prédit adéquatement un réseau, cela
suggère que sa structure est régie par les mécanismes de ces variables. Si nous
savons que le nombre d'espèces est régi (notamment) par des mécanismes de
dispersion, d'immigration, de spéciation et d'extinction
(\cite{Whittaker2001Scale}), et que le nombre d'interactions est régi
(notamment) par des mécanismes liés à la rencontre (\cite{Canard2012Emergence},
\cite{Canard2014Empirical}) et la correspondance de traits
(\cite{Bolnick2011Why}, \cite{Gravel2013Inferring}, \cite{Stouffer2011Role}) des
individus, cela suggère que la structure du réseau dépend de ces mécanismes
importants. Les mécanismes écologiques sont donc implicites dans la théorie,
étant exprimés à travers les valeurs des variables d'état. Deuxièmement, à
l'instar de METE (\cite{Harte2014Maximum}), la théorie de l'entropie maximale
des réseaux trophiques peut être utilisée comme modèle nul
...
mécanismes. Cette théorie ne permet pas d'identifier directement les mécanismes
écologiques importants, mais peut orienter notre recherche vers ceux déterminant
les variables d'état et les erreurs de prédiction. Nous devons utiliser nos
connaissances préalables, effectuer des expériences écologiques et/ou développer
de nouvelles versions de la théorie pour bien décrire et comprendre les
mécanismes précis.
...
décrit d'abord les déterminants écologiques et sources d'incertitude des réseaux
...
L'article~\ref{art:article1} présente les déterminants écologiques et sources
...
quantifier et, le cas échéant, de les réduire. Par ailleurs, cet article
explique que ces deux types de réseaux diffèrent quant à leurs variables
écologiques sous-jacentes, la réalisation des interactions locales étant
conditionnelle à un plus grand nombre de variables, incluant leur faisabilité
...
réseaux trophiques. Il peut être perçu comme un travail de traduction de la
théorie de l'entropie maximale des graphes aléatoires aux réseaux trophiques,
bien que les prédictions de ces théories puissent porter sur des objets quelque
peu différents. En effet, la théorie de l'entropie maximale des graphes
aléatoires prédit des distributions de probabilité sur des graphes en présence
de certaines contraintes (\cite{Bianconi2007Entropy},
\cite{Bianconi2009Entropy}, \cite{Peixoto2012Entropy},
\cite{Coolen2017Generating}), alors que les versions de la théorie de l'entropie
maximale des réseaux trophiques développées dans cet article prédisent les
\textit{propriétés} d'un réseau. L'article~\ref{art:article2} met de l'avant
deux versions de la théorie de l'entropie maximale des réseaux trophiques: la
première utilise le nombre d'espèces et le nombre d'interactions comme variables
d'état pour prédire la distribution jointe de degrés et la matrice d'adjacence,
alors que la seconde utilise la distribution jointe de degrés comme variable
d'état pour prédire la matrice d'adjacence. Dans ces deux versions, la
distribution jointe de degrés a été prédite (le cas échéant) à l'aide d'une
approche dite analytique, alors que la matrice d'adjacence a été prédite à
l'aide d'une approche dite heuristique basée sur l'entropie SVD. Les deux
versions ont été testées à l'aide de réseaux trophiques locaux échantillonnés à
l'échelle mondiale, autant en milieu aquatique que terrestre. La deuxième
version, utilisant la distribution jointe de degrés comme variable d'état,
prédit avec précision plusieurs mesures de la structure des réseaux calculées à
partir de la matrice d'adjacence d'entropie maximale, ce qui suggère que la
structure des réseaux trophiques est principalement déterminée par la
distribution jointe de degrés. Cependant, aucune des deux versions n'a réussi à
prédire fidèlement le niveau trophique maximal et le diamètre des réseaux. Pour
mieux prédire ces mesures, une version améliorée de la théorie pourrait
contraindre la longueur des chaînes trophiques. Pour ce faire, cet article
présente, de manière précise et détaillée, le principe d'entropie maximale et la
méthode des multiplicateurs de Lagrange, permettant à d'autres écologistes de
développer leur propre version de la théorie. 
