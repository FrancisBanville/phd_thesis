...
ce manque de données, mais la diversité des mécanismes écologiques qu'ils
décrivent rend difficile l'identification des mécanismes les plus importants. De
plus, l'incertitude introduite par ces modèles n'est pas toujours bien décrite,
ce qui peut nuire à notre interprétation des réseaux prédits. En reconnaissant
que la structure des réseaux trophiques est écologiquement contrainte, cette
thèse met de l'avant une méthode intrinsèquement probabiliste permettant de
prédire, de manière cohérente et non biaisée, différentes mesures des réseaux
trophiques à partir de ces contraintes. Pour ce faire, elle jette les bases de
la théorie de l'entropie maximale des réseaux trophiques (Trophique-METE), qui
aide à mieux comprendre les déterminants de la structure des réseaux trophiques.
Ne reposant sur aucun mécanisme écologique explicite, cette théorie prédit leur
structure à l'aide du principe d'entropie maximale et d'un ensemble limité de
variables d'état, dont le choix permet de formuler différentes versions de la
théorie. 
...
régionale. Comprendre les propriétés des interactions probabilistes nous aide à
préciser le champs d'application de la théorie.
...
d'identifier les mécanismes écologiques importants à partir de ses erreurs de
prédiction.
...
Predictive models can compensate for this lack of data, but the variety of
ecological mechanisms that they describe makes it difficult to identify the most
important ones. Additionally, the sources of uncertainty associated with these
...
based on these constraints. To do so, it lays the groundwork for the maximum
entropy theory of food webs (Trophic-METE), which enhances our understanding of
the drivers of food-web structure. Without relying on explicit ecological
mechanisms, this theory predicts their structure using the principle of maximum
entropy and a limited set of state variables, the choice of which allows for the
development of different versions of the theory.
...
entropy: an analytical approach using the method of Lagrange multipliers and a
heuristic approach using simulated annealing. I show that the second version,
based solely on the heuristic approach, accurately predicts the structure of
aquatic and terrestrial food webs at a global scale. However, it overestimates
their diameter and maximum trophic level, which suggests that a subsequent
version of the theory constraining trophic chain lengths might be more accurate.
Overall, Trophic-METE facilitates the prediction of food webs by reducing the
amount of information required while helping us identify important ecological
mechanisms from prediction errors.
