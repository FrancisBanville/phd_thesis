...
devons approfondir notre compréhension des réseaux complexes d'interactions
entre espèces tels que les réseaux trophiques. Plusieurs mesures permettent
d'analyser la structure émergente des réseaux trophiques. Ces mesures sont
fortement corrélées les unes avec les autres et certaines d'entre elles, comme
le nombre total d'espèces et d'interactions, contraignent la structure des
réseaux trophiques en réduisant le nombre de configurations possibles. Ces
corrélations et contraintes peuvent faciliter la prédiction des réseaux
trophiques lorsque les données d'interactions sont manquantes. Cependant,
l'incertitude au sein des réseaux d'interactions est encore mal comprise, ce qui
peut nuire à notre interprétation des réseaux prédits en conduisant à un excès
de confiance dans les prédictions. Cette thèse vise à mieux décrire et
quantifier cette incertitude en mettant de l'avant une méthode intrinsèquement
probabiliste permettant de prédire, de manière cohérente et non biaisée,
différentes mesures de la structure des réseaux trophiques. Elle jette les bases
de la théorie de l'entropie maximale des réseaux trophiques (Trophique-METE),
qui permet d'inférer des distributions de probabilité caractérisant la structure
des réseaux trophiques. Prédites par le principe d'entropie maximale, ces
distributions sont dérivée à partir d'un ensemble limité de variables
écologiques, dont le choix permet de formuler différentes versions de la
théorie. 
...
probabiliste qui diffèrent quant à leurs sources d'incertitude~: les
...
probabilité qu'elles soit biologiquement faisables. À l'aide d'un jeu de données
...
régionale. Comprendre les sources d'incertitude des interactions entre espèces
précise le champs d'application de la théorie et guide le développement de
versions basées sur les variables introduisant cette incertitude.
...
contraintes pour inférer la distribution jointe de degrés et la matrice
d'adjacence. La seconde utilise le nombre de proies et de prédateurs par espèce
pour prédire la matrice d'adjacence. Ces versions emploient deux approches
complémentaires prédisant la structure des réseaux avec le principe d'entropie
maximale. L'approche analytique infère directement les distributions de
probabilité à l'aide de la méthode des multiplicateurs de Lagrange, tandis que
l'approche heuristique génère ces distributions par permutations. Je montre que
la seconde version, basée uniquement sur l'approche heuristique, prédit bien la
structure des réseaux trophiques aquatiques et terrestres à l'échelle globale.
Cependant, elle surestime systématiquement le diamètre des réseaux et leur
niveau trophique maximal, ce qui suggère qu'une version subséquente contraignant
la longueur des chaînes trophiques pourrait s'avérer plus précise.
Trophique-METE facilite la prédiction des réseaux trophiques en réduisant le
nombre d'informations requises, tout en mettant en lumière la simplicité des
mécanismes biologiques régissant les réseaux trophiques.
